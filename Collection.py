...
def main():
    ...
    actions = np.array(['hello', 'thanks', 'deaf'])
    no_sequences = 30
    sequence_length = 30
    createDataset(data_dir=DATA_PATH, actions=actions, num_sequences=no_sequences, sequence_length=sequence_length)
        ...
        for action in actions:
            ...
            for sequence in range(no_sequences):
                ...
                for frame_num in range(sequence_length):
                    ...
                    # wait logic
                    if frame_num == 0: 
                        cv2.putText(image, 'Reset Position', (100,100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255, 0), 4, cv2.LINE_AA)
                        cv2.putText(image, f'recording frames for {action} - Video Number {sequence}', (15,12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
